...
bot.use((ctx, next)=>{
    ...
    if(!dados.hasOwnProperty(chatId)) dados[chatId] = []//caso não exista a propriedade chatId dentro do objeto, --> dados[chatId] é responsável por criar a propriedade
...
bot.action(/delete (.+)/, ctx=>{
    ...
    if(indice >= 0) ctx.itens.splice(indice, 1) // manda deletar 1 item a partir do indice
